...
import com.solab.iso8583.parse.LlllvarParseInfo;


public class LlllvarBinaryParseInfo extends LlllvarParseInfo {
	...
	public <T> IsoValue<?> parse(final int field, final byte[] buf, final int pos, final CustomField<T> custom)
		...
		data.setData(ArrayUtils.subarray(buf, pos + 4, pos + 4 + len));
